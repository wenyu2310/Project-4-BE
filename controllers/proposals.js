...
router.put("/:parkId/proposals/:proposalId", verifyToken, async (req, res) => {
  try {
    ...
    if (proposal.userId !== userId) {
      return res
        .status(403)
        .json({ err: "You don't have permission to edit this proposal" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (proposal.userId !== userId) {
        return res
          .status(403)
          .json({ err: "You don't have permission to delete this proposal" });
      }
...
router.get(
  "/:parkId/proposals/:proposalId/likes",
  verifyToken,
  async (req, res) => {
    try {
...
router.delete(
  "/:parkId/proposals/:proposalId/likes/:likeId",
  verifyToken,
  async (req, res) => {
    try {
      const userId = req.user.id; // Assuming verifyToken middleware adds user object
      ...
      if (!liker) {
        return res
          .status(404)
          .json({ err: "Person in mailing list not found" });
      }

      if (liker.userId !== userId) {
        return res
          .status(403)
          .json({ err: "You don't have permission to delete this like" });
      }
